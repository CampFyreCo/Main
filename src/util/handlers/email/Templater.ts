...
export default class Templater {
	private static TEMPLATES: Map<ValidTemplates, ParsedTemplate> = new Map<ValidTemplates, ParsedTemplate>();
	private static get dir() {
		return `${config.dir.base}/src/util/handlers/email/templates`;
	}
	...
	static parseString(str: string, replacers: AnyObject) {
		let v = String(str);
		Object.entries(replacers).map(([key, value]) => v = v.replace(new RegExp(`(%${key}%)`, "g"), String(value)));
		return v;
	}
